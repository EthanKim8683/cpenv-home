		...
		sack(
			...
			[&](int a) -> void {
				for (auto b : subtract[a]) {
					cost[b] -= ft2.query(P[b]) +
							   (ft3.query(N + 1) - ft3.query(P[b])) * P[b];
					cost[b] += ft4.query(P[b]) +
							   (ft5.query(N + 1) - ft5.query(P[b])) * P[b];
				}
				cost[a] += ft2.query(P[a]) +
						   (ft3.query(N + 1) - ft3.query(P[a])) * P[a];
			},
			adj);
